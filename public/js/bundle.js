(() => {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    function project(x, y, z) {
      ...
      const b = 2 * w * (1 - w);
      const c = w * w;
      pt[0] = a * (maps[0] + maps[1] * x + maps[2] * y + maps[3] * z) + b * px + c * (maps[8] + maps[9] * x + maps[10] * y + maps[11] * z);
      pt[1] = a * (maps[4] + maps[5] * x + maps[6] * y + maps[7] * z) + b * py + c * (maps[12] + maps[13] * x + maps[14] * y + maps[15] * z);
      return x < 0 ? 1 : 2;
    }
    ...
    function frame(ts) {
      ...
      if (W < 2) return;
      const dt = lastTs ? Math.min((ts - lastTs) / 1e3, 0.1) : 0;
      lastTs = ts;
  ...
  document.addEventListener("DOMContentLoaded", function() {
    ...
    function updateCountdown() {
      ...
      if (now >= endDate) {
        ...
        if (conferenceInfo) {
          conferenceInfo.innerHTML = `<p>Og\xF3lnopolska Matematyczna Konferencja Student\xF3w</p>`;
        }
    ...
    allNavLinks.forEach((link) => {
      link.addEventListener("click", function(e) {
        ...
        if (href && href.includes("#") && href !== "#") {
          ...
          if (navList2 && navList2.classList.contains("active-menu")) {
            navList2.classList.remove("active-menu");
          }
        }
      });
    });
  ...
  document.addEventListener("DOMContentLoaded", function() {
    ...
    blocks.forEach((block) => {
      ...
      if (state === "open") {
        ...
        btnContainer.innerHTML = `<a class="gray" aria-disabled="true">Zapisz si\u0119!</a>`;
        termElement.textContent = `Termin zapis\xF3w min\u0105\u0142`;
